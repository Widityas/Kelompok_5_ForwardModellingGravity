# Forward Gravity Modelling 2D - GUI Variation (fixed)
# - Menambahkan interp_var, scale_res (resolusi), serta perbaikan minor lainnya.
# - CSV import/export: format: name,x,z,rho
# - Sumber: point-mass vertical component (g_z)
#
# Simpan sebagai file .py lalu jalankan (butuh tkinter, matplotlib, numpy)
...
DEFAULT_INTERP = True   # True -> bicubic, False -> nearest
...
def compute_field(resolution, extent=200):
...
def update_plot():
    ...
    for (name, x0, z0, rho0) in sources:
        ...
        # position label a bit above center to avoid overlap
...
# -------------------------------
root = tk.Tk()
root.title('Forward Modelling Gravitasi (Interaktif) - Fixed UI')
...
root.grid_columnconfigure(0, weight=1)   # frame kiri
root.grid_columnconfigure(1, weight=3)   # frame kanan
root.grid_rowconfigure(0, weight=1)

# left control panel
frame_left = ttk.Frame(root, padding=10, width=600)
...
entry_name = ttk.Entry(frame_left, width=22); entry_name.grid(row=1, column=1, pady=2)

ttk.Label(frame_left, text='x (m):').grid(row=2, column=0, sticky='e')
entry_x = ttk.Entry(frame_left, width=22); entry_x.grid(row=2, column=1, pady=2)

ttk.Label(frame_left, text='z (m, kedalaman):').grid(row=3, column=0, sticky='e')
entry_z = ttk.Entry(frame_left, width=22); entry_z.grid(row=3, column=1, pady=2)

ttk.Label(frame_left, text='ρ (kg/m³):').grid(row=4, column=0, sticky='e')
entry_rho = ttk.Entry(frame_left, width=22); entry_rho.grid(row=4, column=1, pady=2)
...
# interpolation toggle (was missing before)
ttk.Label(frame_left, text='Interpolation:').grid(row=13, column=0, sticky='e')
interp_var = tk.BooleanVar(value=DEFAULT_INTERP)   # True -> bicubic, False -> nearest
cb_interp = ttk.Checkbutton(frame_left, text='Smooth (bicubic)', variable=interp_var, command=update_plot)
cb_interp.grid(row=13, column=1, pady=2, sticky='w')
