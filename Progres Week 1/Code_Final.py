...
import tkinter as tk                          # modul GUI utama
from tkinter import ttk, messagebox, filedialog  # widget tambahan, dialog pesan & file
import numpy as np                             # operasi numerik dan vektor/matriks
import matplotlib                              # modul utama plotting
import matplotlib.pyplot as plt                # API plotting seperti MATLAB
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg  # menempel plot ke tkinter

# Variabel global & konstanta
sources = []         # menyimpan titik sumber (nama, x, z, rho)
cbar = None          # menyimpan colorbar agar bisa dihapus/update
G = 6.67430e-11      # konstanta gravitasi universal
COLOR_MIN = None     # batas bawah warna (None = autoscale)
COLOR_MAX = None     # batas atas warna (None = autoscale)

# Fungsi perhitungan gravitasi sumber titik
def gravity_anomaly_point(x, y, z, m):        
    r = np.sqrt(x**2 + y**2 + z**2)           # jarak ke sumber
    eps = 1e-20                                # cegah pembagian nol
    r3 = (r + eps)**3                          # r^3
    gz_m_s2 = G * m * z / r3                   # komponen vertikal percepatan gravitasi
    return gz_m_s2 * 1e5                       # konversi ke mGal

# Fungsi menggambar / memperbarui plot
def update_plot():                             
    global cbar, fig, ax_map, ax_profile        # pakai variabel global

    ax_map.cla()                                # bersihkan peta
    ax_profile.cla()                            # bersihkan grafik profil
    ...
    if not sources:                             
        ...
        ax_map.set_xticks([]); ax_map.set_yticks([])  # hilangkan axis
        canvas.draw_idle()
        return

    # grid perhitungan
    x = np.linspace(-200, 200, 600)             # grid sumbu X
    y = np.linspace(-200, 200, 600)             # grid sumbu Y
    X, Y = np.meshgrid(x, y)                    # buat mesh 2D
    Z_total = np.zeros_like(X)                  # inisialisasi anomali total

    # superposisi semua sumber
    for (_, x0, z0, rho0) in sources:           
        Z_total += gravity_anomaly_point(X - x0, Y, z0, rho0)

    # profil anomali pada garis Y=0
    g_profile = np.zeros_like(x)                
    for (_, x0, z0, rho0) in sources:
        g_profile += gravity_anomaly_point(x - x0, 0, z0, rho0)
    ...
    # gambar heatmap (interpolasi halus)
    ...
    canvas.draw_idle()                         # perbarui tampilan

# Fungsi tombol GUI
def clear_inputs():                            # hapus text di input
    entry_name.delete(0, tk.END)
    entry_x.delete(0, tk.END)
    entry_z.delete(0, tk.END)
    entry_rho.delete(0, tk.END)

def add_point():                               # tambah sumber baru
    try:
        ...
        if z_val <= 0: messagebox.showerror("Input error", "Nilai z (kedalaman) harus > 0"); return
        if rho_val == 0: messagebox.showerror("Input error", "Nilai ρ tidak boleh 0"); return
        sources.append((name_val, x_val, z_val, rho_val))
        listbox.insert(tk.END, f"{name_val}: x={x_val:.1f}, z={z_val:.1f}, ρ={rho_val:.0f}")
        clear_inputs(); update_plot()
    except ValueError:
        messagebox.showerror("Input error", "Masukkan angka yang valid.")

def delete_selected():                         # hapus sumber dipilih
    sel = listbox.curselection()
    if not sel: messagebox.showerror("Error", "Pilih item dulu!"); return
    idx = sel[0]; sources.pop(idx); listbox.delete(idx); update_plot()

def clear_points():                            # hapus semua sumber
    sources.clear(); listbox.delete(0, tk.END); update_plot()

def on_select(event):                          # klik listbox -> isi input
    sel = listbox.curselection()
    if not sel: return
    idx = sel[0]; name, x0, z0, rho0 = sources[idx]
...
def update_selected():                         # update sumber terpilih
    sel = listbox.curselection()
    if not sel: messagebox.showerror("Error", "Pilih item dulu!"); return
    idx = sel[0]
    try:
        ...
        z_val = float(entry_z.get())
        rho_val = float(entry_rho.get())
    except ValueError: messagebox.showerror("Input error", "Masukkan angka valid!"); return
    sources[idx] = (name_val, x_val, z_val, rho_val)
    listbox.delete(idx); listbox.insert(idx, f"{name_val}: x={x_val:.1f}, z={z_val:.1f}, ρ={rho_val:.0f}"); update_plot()

def save_plot():                               # simpan gambar ke PNG
    fname = filedialog.asksaveasfilename(defaultextension=".png",
                                         filetypes=[("PNG image","*.png"), ("All files","*.*")],
                                         title="Save plot as...")
    if not fname: return
    try: fig.savefig(fname, dpi=150); messagebox.showinfo("Saved", f"Menyimpan gambar ke:\n{fname}")
    except Exception as e: messagebox.showerror("Error", f"Gagal menyimpan: {e}")
...
root.title("Forward Modelling Gravitasi (Interaktif, Multi-Sumber)")  # judul aplikasi
try: root.state("zoomed")                      # buka window full
except Exception: pass
